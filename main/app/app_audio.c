...
void audio_record_save(int16_t *audio_buffer, int audio_chunksize)
{
    ...
    if (record_flag) {
        ...
        ESP_LOGI(TAG, "CWG================================sizeof wav_header: %d", sizeof(wav_header_t));
        ESP_LOGI(TAG, "CWG================================MAX_FILE_SIZE - sizeof(wav_header_t): %u ", (MAX_FILE_SIZE - sizeof(wav_header_t)));
        ESP_LOGI(TAG, "CWG================================record_save===record_total_len: %" PRIu32 " ", record_total_len);
...
static esp_err_t audio_record_stop()
{
    ...
    ESP_LOGI(TAG, "CWG================================record_total_len");
    ESP_LOGI(TAG, "### record Stop,record_total_len: %" PRIu32 " %" PRIu32 "K", \
             record_total_len, \
             record_total_len / 1024);

    ESP_LOGI(TAG, "CWG================================file_total_len: %" PRIu32 " ", file_total_len);
...
// #endif
        ...
        if (ESP_MN_STATE_TIMEOUT == result.state) {
            ...
            if (WIFI_STATUS_CONNECTED_OK == wifi_connected_already()) {
                ...
                // cwg
                ESP_LOGI(TAG, "CWG====MN_STATE_TIMEOUT=============================record_total_len: %" PRIu32 " ", record_total_len);
                start_openai((uint8_t *)record_audio_buffer, record_total_len);


            } else {
                // cwg
                ESP_LOGE(TAG, "CWG=================================wifi disconnected, online function disabled!");
        ...
        if (WAKENET_DETECTED == result.wakenet_mode) {
            ...
            ESP_LOGI(TAG,"CWG================ECHO_EN_WAKE：ding");
